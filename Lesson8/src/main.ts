...
const isTrueInterface = <T>(value: T): BoolCheck<T> => {
...
//!extends
//In TypeScript, the extends keyword is used in the context of generics to specify constraints on the type parameter.
//When you see extends in a generic type parameter declaration like <T extends SomeType>, it means that T must be a subtype of (or compatible with) SomeType.
interface HasID {
  id: number;
}
...
const State = new StateOBJ<(string | number | boolean)[]>([15]); //this will make sure it to go specified type
